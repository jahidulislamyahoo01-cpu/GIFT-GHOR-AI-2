...
export const AdminWhatsAppView: React.FC<AdminWhatsAppViewProps> = ({ authToken, showToast }) => {
  ...
  const [phoneNumberInput, setPhoneNumberInput] = useState('01522126525');
    ...
    try {
      ...
      if (data.success) {
        if (data.whatsappState) {
          ...
          if (data.whatsappState.whatsappNumber) {
  ...
  // Confirm pairing / Link device success
                ...
                <div className="space-y-6">
                  <div className="bg-neutral-950 border border-neutral-800 rounded-xl p-5 space-y-4">
                    <label className="block text-xs font-semibold text-neutral-300">
                      আপনার WhatsApp মোবাইল নম্বর ইনপুট দিন (দেশীয় কোডসহ):
                    </label>
                    <div className="flex gap-2">
                      <div className="relative flex-1">
                        ...
                        <input
                          ...
                          placeholder="01522126525"
                          className="w-full bg-neutral-900 border border-neutral-700 text-white rounded-xl pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:border-emerald-500"
                        />
                      </div>
                      <button
                        onClick={handleGeneratePairingCode}
                        disabled={isGeneratingCode}
                        className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-semibold text-xs md:text-sm rounded-xl transition shadow-md flex items-center gap-1.5 shrink-0"
                      >
                        {isGeneratingCode ? (
                  ...
                  <div className="bg-neutral-950/80 border border-neutral-800 rounded-xl p-4 space-y-3">
                    <h3 className="text-sm font-bold text-emerald-400 flex items-center gap-2">
                      <ShieldCheck className="w-4 h-4" />
                      ৮-ডিজিট কোড দিয়ে লিঙ্ক করার নিয়ম:
                    </h3>
                    <ol className="text-xs text-neutral-300 space-y-2.5 list-decimal pl-4">
