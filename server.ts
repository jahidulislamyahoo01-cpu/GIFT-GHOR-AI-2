  ...
  for (const modelName of candidateModels) {
    for (let k = 0; k < keyCount; k++) {
      ...
      try {
        ...
        // Fast 4.5s timeout per attempt so response is instant
        const timeoutPromise = new Promise((_, reject) =>
          setTimeout(() => reject(new Error(`Timeout (${modelName})`)), 4500)
        );
...
app.post('/api/chat/message', async (req, res) => {
  ...
  try {
    ...
    let botReplyText = await callGeminiAI(chatHistory, systemInstruction, 450, 0.7);
...
app.post('/api/admin/whatsapp/connect-qr', adminAuthMiddleware, (req, res) => {
  if (!DB.whatsappState) {
    DB.whatsappState = {
      connected: false,
      whatsappNumber: '01522126525',
  ...
  // Generate dynamic QR code payload string representing WhatsApp Web Multi-Device session
  const qrToken = `2@gg_wa_session_${Date.now()}_${Math.random().toString(36).substring(2, 12)}==,GG_BOT,8c66ff39`;
...
// 3. Initiate Phone Number Pairing Code Connection
app.post('/api/admin/whatsapp/connect-phone', adminAuthMiddleware, (req, res) => {
  const { phoneNumber } = req.body;
  if (!phoneNumber || String(phoneNumber).trim().length < 8) {
    return res.status(400).json({ error: 'অনুগ্রহ করে একটি সঠিক WhatsApp মোবাইল নম্বর দিন।' });
  }

  const cleanPhone = String(phoneNumber).trim();

  // Generate 8-character pairing code like "GG88-W4P2"
  ...
  DB.whatsappState.pairingCode = pairingCode;
  DB.whatsappState.sessionStatus = 'pairing_ready';
  
  if (!DB.adminSettings) {
    DB.adminSettings = { twoFactorEnabled: false, twoFactorEmail: 'giftghor6525@gmail.com' };
  }
  DB.adminSettings.whatsappNumber = cleanPhone;
  ...
  res.json({
    ...
    whatsappNumber: cleanPhone,
    sessionStatus: 'pairing_ready',
    message: `8-character pairing code ${pairingCode} generated! Open WhatsApp -> Linked Devices -> Link with phone number instead.`,
  });
});

// 4. Confirm Pairing / Device Link Success
app.post('/api/admin/whatsapp/confirm-pair', adminAuthMiddleware, (req, res) => {
  ...
  res.json({
    success: true,
    message: '🎉 WhatsApp account successfully connected as Linked Device!',
    whatsappState: DB.whatsappState,
  });
});

// 5. Disconnect WhatsApp Device
app.post('/api/admin/whatsapp/disconnect', adminAuthMiddleware, (req, res) => {
  if (!DB.whatsappState) {
    DB.whatsappState = {
      connected: false,
      whatsappNumber: '01522126525',
...
app.post('/api/admin/whatsapp/settings', adminAuthMiddleware, (req, res) => {
  ...
  if (whatsappNumber) DB.whatsappState.whatsappNumber = String(whatsappNumber).trim();
...
// 8. Admin Manual Reply to WhatsApp Contact
app.post('/api/admin/whatsapp/send-reply', adminAuthMiddleware, (req, res) => {
...
app.post('/api/admin/crawler/start', adminAuthMiddleware, async (req, res) => {
  ...
  try {
    ...
    try {
      const response = await axios.get(targetUrl, { timeout: 25000, headers });
...
async function crawlGiftGhor() {
  ...
  try {
    ...
    const sitemapRes = await axios.get('https://giftghor.world/api/sitemaps.xml', { timeout: 25000, headers });
    ...
    for (const url of urls) {
      ...
      try {
        const response = await axios.get(url, { timeout: 25000, headers });
        ...
        // Remove script, style, noscript, svg, nav, footer, header to get clean product content
        $('script, style, noscript, svg, nav, footer, header').remove();
        const bodyText = $('body').text().replace(/\s+/g, ' ').trim().substring(0, 1800);
        
        // Build clean, high-value product knowledge
        ...
        console.warn(`[Crawler] Note: Failed to fetch ${url} directly (${err?.message || 'timeout'}). Checking catalog fallback...`);
        // Fallback using product catalog if fetch timed out
        if (matchingProduct) {
    ...
    console.warn('[Crawler] Could not reach external sitemap feed:', err.message);
  }
}
